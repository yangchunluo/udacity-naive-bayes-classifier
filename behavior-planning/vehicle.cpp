...
Vehicle::Vehicle(int lane, int s, float v, float a, string state) {
    this->lane = lane;
    this->s = s;
    this->v = v;
    this->a = a;
...
vector<Vehicle> Vehicle::choose_next_state(map<int, vector<Vehicle>> predictions) {
    ...
    */
    vector<string> candidate_states = successor_states();

    vector<float> costs;
    vector<vector<Vehicle>> trajectories;

    for (auto const &state : candidate_states) {
        auto trajectory = generate_trajectory(state, predictions);
        if (!trajectory.empty()) {
            trajectories.emplace_back(trajectory);
            costs.emplace_back(calculate_cost(*this, predictions, trajectory));
        }
    }
    ...
    */
    ...
    if (get_vehicle_ahead(predictions, lane, vehicle_ahead)) {
        ...
        if (get_vehicle_behind(predictions, lane, vehicle_behind)) {
            ...
            auto max_velocity_in_front = (float) (
    ...
    new_position = (float) (this->s + new_velocity + new_accel / 2.0);
...
vector<Vehicle> Vehicle::constant_speed_trajectory() {
    ...
    */
    ...
    vector<Vehicle> trajectory = {Vehicle(this->lane, this->s, this->v, this->a, this->state),
                                  Vehicle(this->lane, (int) next_pos, this->v, 0, this->state)};
    return trajectory;
}
...
vector<Vehicle> Vehicle::keep_lane_trajectory(map<int, vector<Vehicle>> predictions) {
    ...
    */
    ...
    trajectory.emplace_back(Vehicle(this->lane, (int) new_s, new_v, new_a, "KL"));
    return trajectory;
}
...
vector<Vehicle> Vehicle::prep_lane_change_trajectory(string state, map<int, vector<Vehicle>> predictions) {
    ...
    */
    ...
    trajectory.emplace_back(Vehicle(this->lane, new_s, new_v, new_a, state));
    return trajectory;
}
...
vector<Vehicle> Vehicle::lane_change_trajectory(string state, map<int, vector<Vehicle>> predictions) {
    ...
    */
    ...
    trajectory.emplace_back(Vehicle(this->lane, this->s, this->v, this->a, this->state));
...
void Vehicle::increment(int dt) {
    this->s = (int) position_at(dt);
}

float Vehicle::position_at(int t) {
    return (float) (this->s + this->v * t + this->a * t * t / 2.0);
}
...
vector<Vehicle> Vehicle::generate_predictions() {
    ...
    */
    ...
    for (int i = 0; i < horizon; i++) {
        ...
        predictions.emplace_back(Vehicle(this->lane, (int) next_s, next_v, 0, "CS"));
    }
    return predictions;
...
void Vehicle::configure_ego(int target_speed, int num_lanes, int goal_lane, int goal_dist, int max_acc) {
    /*
